...
export class LayoutManager {
    ...
    createLayout() {
        ...
        layoutContainer.style.position = 'fixed';  // Changed from absolute to fixed
    ...
    setupEventListeners() {
        ...
        window.addEventListener('resize', () => {
            if (this.isExpanded) {
                const mixcloudHeight = this.layout.mixcloudContainer?.classList.contains('visible') ? 120 : 0;
    ...
    updateLayout(textHeight) {
        ...
        // Ensure text content is scrolled to top when resizing
        if (adjustedTextHeight === this.minHeight) {
    ...
    async showContent(title, textPath, withMixcloud = false) {
        try {
            ...
            const processedText = text.trim();  // Use trim() instead of regex
